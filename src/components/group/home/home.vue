            ...
            <div class="group__item-simple" v-if="group">
                ...
                <div class="group__item-in">
                    ...
                    <span><i class="iconfont icon-bisai2"></i>
                        <a :href="'/group/match.html?groupId='+group.id" :class="[ group.matchCount ? 'user-link': 'tip-default']">{{group.matchCount}}</a>场</span>
                    <span><i class="iconfont icon-jiangpai"></i><a href="" :class="[ group.matchCount ? 'user-link' : 'tip-default']">{{group.rankCount}}</a>期</span>
                </div>
            </div>
        </div>
        <!-- 爬爬群信息 -->
...
<script>
    ...
    export default {
        ...
        computed: {
            group () {
                console.log('computed', new Date().getTime(), this.$store.state.group);
